...
async def handle_message(client: userbot, message: Message):
    ...
    await userbot.send_message(LOG_GROUP, f"{sender_name} sent a message with username @{user_unme}: {user_msg}")
    ...
    if not is_approved(user_id) and message_count >= WARNING_LIMIT:
